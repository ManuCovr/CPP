...
template <typename Container>
PmergeMe<Container>::PmergeMe() : start_(0), end_(0) {}

template <typename Container>
PmergeMe<Container>::PmergeMe(const PmergeMe &other)
    : container(other.container), start_(other.start_), end_(other.end_) {}
    ...
    if (pend.size() == 1) {
        ...
        while (!pend.empty()) {
            ...
            while (idx) {
                ...
                if (static_cast<size_t>(Jacobsthal(jc + count)) - decrease <= main.size())
...
void PmergeMe<Container>::sort_time() {
    ...
    std::cout << "Time to process a range of " << container.size() << " elements with " << container_type() << " : " << std::fixed << std::setprecision(5) << elapsedTime << " us" << std::endl;
}

template <typename Container>
double PmergeMe<Container>::time() {
    return static_cast<double>(end_ - start_) / CLOCKS_PER_SEC * 1000000.0;
}
